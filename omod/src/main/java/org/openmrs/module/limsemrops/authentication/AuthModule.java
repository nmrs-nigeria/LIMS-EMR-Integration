...
public class AuthModule {
	...
	private String requestToken() throws UnirestException, SQLException {
		...
		HttpResponse<String> response = Unirest.post(this.endPoint).body(requestBody).asString();
		String responseBody = response.getBody();
		int statusCode = response.getStatus();
		//now, if its an initial request, we should strip out the new password and use the correct method
		if (isInitialRequest) {
			//find the new password and extract it from the JWT, then request a new, sanitized JWT
			...
			if (statusCode != 200) {
				//this is not an OK, so likely we should invalidate our current token and redo
